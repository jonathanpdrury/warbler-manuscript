...
simdata.mass<-fastBM(tree,a=0,sig2=0.015852 , bounds=c(-1.325469,3.318858 ),nsim=N)
...
simdata.bill<-fastBM(tree,sig2=0.019364 ,a= 0,bounds=c(-1.352945,3.518544), nsim=N)
...
R<-0.015852 #rate parameter to use min=0.015852 max=0.038728
Al<-5*R #alpha to use, 0.1*R & 5*R for each R value
#plumage similarity 
simdata.1<-fastBM(tree,sig2= R,a= 0,alpha=Al,theta=0,nsim=N)
simdata.2<-fastBM(tree,sig2= R,a= 0,alpha=Al,theta=0,nsim=N)
simdata.plumarray<-array(dim=c(44,N,2)) #44 is number of tips, N is number of simulations, 2 is number of variables simulated
...
simdata.1<-fastBM(tree,sig2= R,a= 0,alpha=Al,theta=0,nsim=N)
simdata.2<-fastBM(tree,sig2= R,a= 0,alpha=Al,theta=0,nsim=N)
simdata.humsongarray<-array(dim=c(44,N,2)) #44 is number of tips, N is number of simulations, 2 is number of variables simulated
...
simdata.1<-fastBM(tree,sig2= R,a= 0,alpha=Al,theta=0,nsim=N)
simdata.2<-fastBM(tree,sig2= R,a= 0,alpha=Al,theta=0,nsim=N)
simdata.songarray<-array(dim=c(44,N,2)) #44 is number of tips, N is number of simulations, 2 is number of variables simulated
...
#guild overlap (0.3442623 [105] are 1's)
simdata.guild<-fastBM(tree,sig2=R,a=0,nsim=N)
variable6.matrix<-var.matrix(simdata.guild)

#this loop pulls out 105 differences for each run of the code for identifying as 1's (note: this loop is computationally expensive, and takes several minutes to run)
int.matrix<-matrix(nrow=305,ncol=N)
for(j in 1:N){
	i=1
	while(i<306){
		int.matrix[i,j]<-ifelse(variable6.matrix[i,j]<quantile(variable6.matrix[,j],c(0.3442623)),1,0) 
		i=i+1
		}
	}
variable6.matrix<-int.matrix
	
#hybridization, 24 pairs, 0.07868852
...
for(j in 1:N){
	i=1
	while(i<306){
		int.matrix[i,j]<-ifelse(variable7.matrix[i,j]<quantile(variable7.matrix[,j],c(0.07868852)),1,0)
		i=i+1
		}
	}
...
var2.95CI<-quantile(results.matrix[2,],c(0.025,0.975))
var3.95CI<-quantile(results.matrix[3,],c(0.025,0.975))
var4.95CI<-quantile(results.matrix[4,],c(0.025,0.975))
var5.95CI<-quantile(results.matrix[5,],c(0.025,0.975))
var6.95CI<-quantile(results.matrix[6,],c(0.025,0.975))
var7.95CI<-quantile(results.matrix[7,],c(0.025,0.975))
var8.95CI<-quantile(results.matrix[8,],c(0.025,0.975))
var9.95CI<-quantile(results.matrix[9,],c(0.025,0.975))
...
for (i in 1:N){
	m1<-glm(data$all.IT~variable2.matrix[,i]+variable4.matrix[,i]+data$Zsyn+variable7.matrix[,i],family="binomial")
	results.matrix[1,i]<-as.numeric(summary(m1)[5]) #this pulls out the aic score
...
var5.CI90<-quantile(results.matrix[5,],c(0.05,0.95))
...
var2.CI95<-quantile(results.matrix[2,],c(0.025,0.975))
var3.CI95<-quantile(results.matrix[3,],c(0.025,0.975))
var4.CI95<-quantile(results.matrix[4,],c(0.025,0.975))
var5.CI95<-quantile(results.matrix[5,],c(0.025,0.975))
